...
### 방법 1. Python 콘솔에서 실행 (간단한 방법)

1. QGIS 실행 후 상단 메뉴 `Plugins(플러그인) > Python Console(파이썬 콘솔)`을 클릭합니다. (단축키: `Ctrl+Alt+P`)
2. 콘솔 창 상단의 **"코드 에디터 표시(Show Editor)"** 아이콘을 클릭합니다. (종이와 연필 모양 아이콘)
3. 에디터 창 상단의 **"파일 열기"** 아이콘으로 `pyqgis_suitability_script_v2.py` 파일을 불러옵니다.
4. 스크립트 상단의 경로·필드명 변수를 수정합니다.
   - `PATH_DONG`, `PATH_SHELTER`, `PATH_ROAD`, `OUTPUT_PATH`: 실제 파일 경로로 변경
   - `POP_COUNT_FIELD`: 실제 인구수 필드명 확인 (기본값 `pop`)
   - `W_POP`, `W_SHELTER`, `W_ROAD`: 팀에서 확정한 가중치 값 확인
   - 좌표계가 위경도(EPSG:4326)라면 재투영 코드 주석을 해제
5. 에디터 상단의 **"스크립트 실행(Run Script)"** 버튼(▶)을 클릭합니다. (단축키: `Ctrl+Shift+E` 또는 `F5`, 버전에 따라 다름)
6. 콘솔 하단에 `[1]`, `[2]` 순서로 진행 로그가 출력되는지 확인합니다.
7. 완료되면 QGIS 레이어 패널에 `행정동`, `기존대피소`, `도로`, `입지적합성_결과` 레이어가 자동으로 추가됩니다.
8. 오류가 발생하면 콘솔에 표시되는 메시지(파일 경로 오류, 필드명 오류 등)를 확인하고, 해당 변수를 수정한 뒤 다시 실행합니다.

### 방법 2. Processing 스크립트 편집기에서 실행 (알고리즘으로 등록하고 싶을 때)

1. 상단 메뉴 `Processing > 툴박스`를 엽니다. (보이지 않으면 `보기 > 패널 > 처리 툴박스`)
2. 툴박스 하단의 **스크립트(Scripts)** 항목을 우클릭해 **새 스크립트 생성(Create New Script)** 또는 **기존 스크립트 열기(Open Existing Script)**로 `pyqgis_suitability_script_v2.py`를 불러옵니다.
3. 스크립트 편집기 상단의 **실행(Run)** 버튼(▶)을 클릭합니다.
4. 이 방식은 스크립트를 Processing 알고리즘 형태로 저장해두고 툴박스에서 반복 실행하고 싶을 때 유용합니다. 반복 사용 빈도가 높다면 그래픽 모델러로 만드는 편이 더 편리합니다.

### 실행 전 체크리스트

- [ ] 행정동·기존 대피소·도로 shp 파일 경로가 실제 파일 위치와 일치하는지 확인
- [ ] 실제 레이어명·필드명을 속성 테이블에서 직접 확인 (문서·스크립트의 예시명과 다를 수 있음)
- [ ] 행정동 레이어의 인구수 필드명이 `pop`이 맞는지 속성 테이블에서 확인 (다르면 `POP_COUNT_FIELD` 값 수정)
- [ ] 행정동·기존 대피소·도로 레이어의 좌표계가 서로 동일하거나 QGIS가 자동 변환할 수 있는 상태인지 확인
- [ ] 출력 폴더(`OUTPUT_PATH`의 상위 폴더)가 실제로 존재하는지 확인 (없으면 오류 발생)
- [ ] `processing` 모듈을 사용하려면 QGIS가 정상적으로 초기화된 상태(일반 QGIS 데스크톱 실행 상태)여야 함

### 자주 발생하는 오류

| 오류 상황 | 원인 | 해결 방법 |
|---|---|---|
| `[오류] 행정동 레이어를 불러오지 못했습니다` | 파일 경로 오타, 파일 미존재 | `PATH_DONG` 등 경로 재확인 |
| `KeyError` 또는 필드 없음 오류 | 실제 필드명과 스크립트 변수명 불일치 | 속성 테이블에서 필드명 확인 후 `POP_COUNT_FIELD` 등 수정 |
| 면적·거리 값이 비정상적으로 크거나 작음 | 레이어가 위경도(EPSG:4326) 상태 | 재투영 코드 주석 해제 후 재실행 |
| 출력 파일 저장 실패 | `OUTPUT_PATH`의 폴더가 존재하지 않음 | 해당 폴더를 미리 생성하거나 존재하는 경로로 변경 |
