...
PyQGIS는 QGIS 내부 기능을 Python 코드로 호출할 수 있도록 제공하는 API(라이브러리)이다. QGIS 자체는 C++로 작성되어 있으나, 이 기능들을 Python에서도 그대로 사용할 수 있도록 QGIS의 기능들을 미리 함수 또는 클래스 형태로 만들어 놓은 것이다. 즉 QGIS 메뉴를 클릭해서 수행하던 작업 대부분을 같은 결과를 내는 코드 몇 줄로 대체할 수 있다.
...
QGIS 콘솔의 코드 편집기(편집기 표시하기 버튼으로 활성화), 또는 `프로세싱 → 도구상자 → 스크립트로 새 스크립트 만들기`에서 `.py` 파일로 저장해 실행하는 방식이다. 콘솔과 마찬가지로 QGIS 내부에서 실행되므로 `iface`를 사용할 수 있으나, 이후 QGIS를 다시 열 때 자동 실행되지는 않는다.
...
**처음 배우는 단계에서는 ①(콘솔) 또는 ② Processing 스크립트 (Script Editor) 사용하는 것을 권장한다.** 독립 실행형 스크립트는 설치 경로·환경변수 설정이 추가로 필요해 난이도가 높으므로, 콘솔에 충분히 익숙해진 뒤에 다루는 것이 안전하다.
